function Score = flappybirdMPC2(rn)
if (~nargin)
    rn = 1;
end
rng(rn);  %random number generator
switch rn  %name file for collecting data
    case 1
        file1 = 'FlappyDataMPC90x1Seed1max.txt';
    case 2
        file1 = 'FlappyData85MPC85x1Seed2.txt';
    case 3
        file1 = 'FlappyData85MPC85x1Seed3.txt';
    case 4
        file1 = 'FlappyData85MPC85x1Seed4.txt';
    case 5
        file1 = 'FlappyData85MPC85x1Seed5.txt';
    case 6
        file1 = 'FlappyData85MPC85x1Seed6.txt';
    case 7
        file1 = 'FlappyData85MPC85x1Seed7.txt';
    case 8
        file1 = 'FlappyData85MPC85x1Seed8.txt';
    case 9
        file1 = 'FlappyData85MPC85x1Seed9.txt';
    case 10
        file1 = 'FlappyData85MPC85x1Seed10.txt';
end
%% System Variables:
...
SecondP = 2;
ThirdP = 3;
Tubes.ScreenX = [90 170 250]-2; % The middle of each tube
Tubes.VOffset = ceil(rand(1,3)*105); 
Tubes.VOffset = [50 50 50]; 
setPointX = 1;
setPointY = Tubes.VOffset(setPointX);
Best = 0;
%% -- Game Logic --
initVariables();
initWindow();
...
% delete(logo_and);
% pause(1);
% write y position and tube height data to file
%fidY = fopen('LastHeight.txt','w+');
%fidVOffset = fopen('VOffset.txt','w+');
%fidsetPointY = fopen('setPointY.txt.','w+');
tubespace = 80;  %horizontal spacing of tubes
n = 90; %planning horizon
nupdate = 1;  %control horizon
xnew = zeros(3*n,1);
j = nupdate+1;
h1 = plot(45,45);
h2 = plot(45,45);
...
while 1
    ...
    while (curTime >= ((CurrentFrameNo) * GAME.FRAME_DURATION) && loops < GAME.MAX_FRAME_SKIP)
        ...
        if FlyKeyStatus  % If left key is pressed     
            if ~gameover
%                Bird.SpeedY = .5*(-(setPointY +22)-(Bird.ScreenPos(2)-176)) % -2.5;
                BirdPos = -(Bird.ScreenPos(2)-176);
                TubePos = setPointY + 10;
%                 if (BirdPos < TubePos)
%                      Jinit = 1;
%                  end
                
               if j > nupdate
                   [J,exitflag,bcon,tcon,opttime,PosYnew,fval,xnew] = FlappyMixIntOptFunMPC2(Tubes.FrontP,Tubes.VOffset,...
                       Bird.ScreenPos,Tubes.ScreenX,Bird.SpeedY,n,SecondP,ThirdP,tubespace);
                   
                   %write data to file
%                    FlappyData = [fval, opttime, xnew', Tubes.FrontP,Tubes.VOffset,...
%                        Bird.ScreenPos,Tubes.ScreenX,Bird.SpeedY,n,SecondP,bcon',tcon',ThirdP];
%                       dlmwrite(file1,FlappyData,'-append');
               ...
               % plot path
               % slows down the animation, comment out if you don't want to
               % see it
               if Score < 10
               set(h1,'Visible','off')
               set(h2,'Visible','off')
               set(h3,'Visible','off')
               h1 = plot(tplot+46-j,yplot,'r','LineWidth',1); 
               h2 = plot(tplot+46-j,bcon,'g','LineWidth',1.5);
               h3 = plot(tplot+46-j,tcon,'g','LineWidth',1.5);
               end
               if Score == 10
               set(h1,'Visible','off')
               set(h2,'Visible','off')
...
%                 end
        ...
        if Flags.PreGame
            ...
            if ~gameover
            %fwrite(fidY,Bird.ScreenPos(2));     %write y position to file
            end                  
            Bird.ScrollX = Bird.ScrollX + 1;
            if ~gameover
                scrollTubes(1);
                %fwrite(fidVOffset,Tubes.VOffset(Tubes.FrontP));
                setPointY = Tubes.VOffset(setPointX);
                %fwrite(fidsetPointY,setPointY);
                if Tubes.ScreenX(Tubes.FrontP) <= (Bird.ScreenPos(1)-30)
...
%         scrollTubeLayer(GAME.RESOLUTION(2));   % Entire tube layer
        ...
        SecondP = 2;
        ThirdP = 3;
        Tubes.ScreenX = [90 90+tubespace 90+2*tubespace]-2; % The middle of each tube
        %Tubes.VOffset = ceil(rand(1,3)*105);
        Tubes.VOffset = [50 50 50]; 
        refreshTubes;
        for i = 1:3
...
%% Game Logic
    ...
    function scrollTubes(offset)
        ...
        if Tubes.ScreenX(Tubes.FrontP) <= 15
            Tubes.ScreenX(Tubes.FrontP) = Tubes.ScreenX(Tubes.FrontP) + 3*tubespace;
            Tubes.VOffset(Tubes.FrontP) = ceil(rand*105);
            %Tubes.VOffset(Tubes.FrontP) = 50;
            redrawTube(Tubes.FrontP);
            Tubes.FrontP = mod((Tubes.FrontP),3)+1;
            SecondP = mod((Tubes.FrontP),3)+1;
            ThirdP = mod((Tubes.FrontP),3)+1;
            Flags.NextTubeReady = true;
        end
%         if Tubes.ScreenX(Tubes.FrontP) <=45
%             Tubes.FrontP = mod((Tubes.FrontP),3)+1;
%         end
    end
...
%% --- Math Functions for handling Collision / Rotation etc. ---
    ...
    function addScore()
        if Tubes.ScreenX(Tubes.FrontP) < 47 && Flags.NextTubeReady
